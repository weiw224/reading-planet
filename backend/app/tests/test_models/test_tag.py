...
from sqlalchemy import Column, Integer, String, DateTime, Enum as SQLEnum, ForeignKey, UniqueConstraint, select
...
@pytest.mark.asyncio
async def test_tag_unique_constraint_allows_same_name_different_category(test_session: AsyncSession):
    tag1 = Tag(
        name="五年级",
        category=TagCategoryEnum.GRADE
    )
...
async def test_tag_repr(test_session: AsyncSession):
    tag = Tag(
        name="神话",
        category=TagCategoryEnum.CULTURE
    )
...
async def test_tag_display_order(test_session: AsyncSession):
    ...
    result = await test_session.execute(
        select(Tag).where(Tag.category == TagCategoryEnum.GRADE).order_by(Tag.display_order)
    )
